...
describe 'param groups' do
  ...
  it 'should replace parameter name in a group when it comes from concern' do
    Apipie['overridden_concern_resources#update'].params.key?(:user).should == true
  end

  it "shouldn't replace parameter name in a group redefined in the controller" do
    Apipie['overridden_concern_resources#create'].params.key?(:concern).should == true
    Apipie['overridden_concern_resources#create'].params.key?(:user).should == false
  end

  it "shouldn't replace name of a parameter defined in the controller" do
    Apipie['overridden_concern_resources#custom'].params.key?(:concern).should == true
    Apipie['overridden_concern_resources#custom'].params.key?(:user).should == false
  end
end
