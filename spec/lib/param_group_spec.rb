...
describe "param groups" do
  ...
  it "lets reuse the params description in more actions" do
    user_create_desc = MoonRaker["users#create"].params[:user]
    user_create_params = user_create_desc.validator.params_ordered.map(&:name)

    user_update_desc = MoonRaker["users#update"].params[:user]
  ...
  it "allows using groups is nested param descriptions" do
    user_create_desc = MoonRaker["users#update"].params[:user]
    user_create_params = user_create_desc.validator.params_ordered.map(&:name)
  ...
  context "hash param" do
    it "influences only its childs" do
      expect(MoonRaker["users#create"].params[:user].required).to be true
      expect(MoonRaker["users#update"].params[:user].required).to be true
    end
  end

  it "lets you reuse a group definition from different controller" do
    arch_v1_desc = MoonRaker["1.0#architectures#create"].params[:architecture]
    arch_v1_params = arch_v1_desc.validator.params_ordered.map(&:name)

    arch_v2_desc = MoonRaker["2.0#architectures#create"].params[:architecture]
  ...
  it "should replace parameter name in a group when it comes from concern" do
    expect(MoonRaker["overridden_concern_resources#update"].params.has_key?(:user)).to eq(true)
  end

  it "shouldn't replace parameter name in a group redefined in the controller" do
    expect(MoonRaker["overridden_concern_resources#create"].params.has_key?(:concern)).to eq(true)
    expect(MoonRaker["overridden_concern_resources#create"].params.has_key?(:user)).to eq(false)
  end

it "shouldn't replace name of a parameter defined in the controller" do
    expect(MoonRaker["overridden_concern_resources#custom"].params.has_key?(:concern)).to eq(true)
    expect(MoonRaker["overridden_concern_resources#custom"].params.has_key?(:user)).to eq(false)
  end
