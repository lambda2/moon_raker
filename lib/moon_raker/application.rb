require 'moon_raker/static_dispatcher'
require 'moon_raker/routes_formatter'
require 'yaml'
require 'digest/md5'
require 'json'
  ...
  class Application
    ...
    class Engine < Rails::Engine
      initializer 'static assets', before: :build_middleware_stack do |app|
        app.middleware.use ::MoonRaker::StaticDispatcher, "#{root}/app/public"
      end
    end
    ...
    def rails_routes(route_set = nil)
      return @rails_routes if route_set.nil? && @rails_routes
    ...
    def routes_for_action(controller, method, args)
      ...
      MoonRaker.configuration.routes_formatter.format_routes(routes, args)
    end
    ...
    def define_method_description(controller, method_name, dsl_data)
      ...
      versions.each do |version|
        ...
        method_description = MoonRaker::MethodDescription.new(method_name, resource_description, dsl_data)
    ...
    def define_resource_description(controller, version, dsl_data = nil)
      ...
      if resource_description
        ...
        resource_description = MoonRaker::ResourceDescription.new(controller, resource_name, dsl_data, version)

        MoonRaker.debug("@resource_descriptions[#{version}][#{resource_name}] = #{resource_description}")
    ...
    def controller_versions(controller)
      ...
      if controller == ActionController::Base || controller.nil?
        return [MoonRaker.configuration.default_version]
      else
    ...
    def get_method_description(resource_name, method_name = nil)
      ...
      elsif resource_name.respond_to? :moon_raker_resource_descriptions
        resource_description = get_resource_description(resource_name)
      else
    ...
    def get_resource_description(resource, version = nil)
      if resource.is_a?(String)
        ...
        resource_name = "#{version}##{resource_name}" if version
    ...
    def recorded_examples
      ...
      @recorded_examples = MoonRaker::Extractor::Writer.load_recorded_examples
    end
    ...
    def to_json(version, resource_name, method_name, lang)
      ...
      url_args = MoonRaker.configuration.version_in_url ? version : ''

      {
        docs: {
          name: MoonRaker.configuration.app_name,
          info: translate(MoonRaker.app_info(version), lang),
          copyright: MoonRaker.configuration.copyright,
          doc_url: MoonRaker.full_url(url_args),
          api_url: MoonRaker.api_base_url(version),
    ...
    def api_controllers_paths
      Dir.glob(MoonRaker.configuration.api_controllers_matcher)
    end

    def reload_documentation
      # don't load translated strings, we'll translate them later
      old_locale = locale
      locale = MoonRaker.configuration.default_locale
      ...
      @checksum = nil if MoonRaker.configuration.update_checksum

      locale = old_locale
    end

    def load_documentation
      if !@documentation_loaded || MoonRaker.configuration.reload_controllers?
    ...
    def compute_checksum
      if MoonRaker.configuration.use_cache?
        file_base = File.join(MoonRaker.configuration.cache_dir, MoonRaker.configuration.doc_base_url)
        all_docs = {}
        ...
        all_docs = MoonRaker.available_versions.inject({}) do |all, version|
          all.update(version => MoonRaker.to_json(version))
        end
      end
    ...
    def active_dsl?
      MoonRaker.configuration.validate? || !MoonRaker.configuration.use_cache? || MoonRaker.configuration.force_dsl?
    end
    ...
    def get_resource_name(klass)
      if klass.class == String
      ...
      elsif MoonRaker.configuration.namespaced_resources? && klass.respond_to?(:controller_path)
        ...
        raise "MoonRaker: Can not resolve resource #{klass} name."
      end
    end

    def locale
      MoonRaker.configuration.locale.call(nil) if MoonRaker.configuration.locale
    end

    def locale=(locale)
      MoonRaker.configuration.locale.call(locale) if MoonRaker.configuration.locale
    end

    def translate(str, locale)
      if MoonRaker.configuration.translate
        MoonRaker.configuration.translate.call(str, locale)
      else
        str
      end
    end
    ...
    def valid_search_args?(version, resource_name, method_name)
      return false unless resource_descriptions.key?(version)
      if resource_name
        return false unless resource_descriptions[version].key?(resource_name)
        if method_name
          resource_description = resource_descriptions[version][resource_name]
    ...
    def get_base_url(version)
      MoonRaker.configuration.api_base_url[version]
    end
    ...
    def get_resource_version(resource_description)
      if resource_description.respond_to? :_version
        ...
        MoonRaker.configuration.default_version
      end
    end
    ...
    def ignored?(controller, method = nil)
      ignored = MoonRaker.configuration.ignored
