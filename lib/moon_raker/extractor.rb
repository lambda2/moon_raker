...
require 'set'
require 'yaml'
require 'moon_raker/extractor/recorder'
require 'moon_raker/extractor/writer'
require 'moon_raker/extractor/collector'

class MoonRaker::Railtie
  initializer 'moon_raker.extractor' do |app|
    ActiveSupport.on_load :action_controller do
      before_action do |controller|
        if MoonRaker.configuration.record
          MoonRaker::Extractor.call_recorder.analyse_controller(controller)
        end
      end
    end
    app.middleware.use ::MoonRaker::Extractor::Recorder::Middleware
    ActionController::TestCase::Behavior.instance_eval do
      include MoonRaker::Extractor::Recorder::FunctionalTestRecording
    end
  end
end
...
module MoonRaker
  module Extractor
    class << self
      def start(record)
        MoonRaker.configuration.record = record
      ...
      def finish
        ...
        case MoonRaker.configuration.record
      ...
      def call_recorder
        Thread.current[:moon_raker_call_recorder] ||= Recorder.new
      end
      ...
      def clean_call_recorder
        Thread.current[:moon_raker_call_recorder] = nil
      end
      ...
      def apis_from_routes
        ...
        @api_prefix = MoonRaker.api_base_url.sub(/\/$/, '')
        populate_api_routes
      ...
      def controller_path(name)
        MoonRaker.api_controllers_paths.detect { |p| p.include?("#{name}_controller.rb") }
      end

      private

      def all_api_routes
        all_routes = MoonRaker.configuration.api_routes.routes.map do |r|
          {
            verb: case r.verb
        ...
        all_routes.find_all do |r|
          r[:path].starts_with?(MoonRaker.api_base_url)
        end
      end
      ...
      def all_apis_from_docs
        resource_descriptions = MoonRaker.resource_descriptions.values.map(&:values).flatten
      ...
      def update_api_descriptions
        ...
        @apis_from_routes.each do |(controller, action), new_apis|
          method_key = "#{MoonRaker.get_resource_name(controller.constantize)}##{action}"
