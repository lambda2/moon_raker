...
module MoonRaker
  class MethodDescription
    ...
    def initialize(method, resource, dsl_data)
      ...
      @full_description = MoonRaker.markup_to_html(desc)

      @errors = dsl_data[:errors].map do |args|
        MoonRaker::ErrorDescription.from_dsl_data(args)
      end

      @see = dsl_data[:see].map do |args|
        MoonRaker::SeeDescription.new(args)
      end
      ...
      @params_ordered = dsl_data[:params].map do |args|
        MoonRaker::ParamDescription.from_dsl_data(self, args)
      end
    ...
    def params_ordered
      all_params = []
      parent = MoonRaker.get_resource_description(@resource.controller.superclass)
    ...
    def doc_url
      crumbs = []
      crumbs << @resource._version if MoonRaker.configuration.version_in_url
      crumbs << @resource._id
      crumbs << @method
      ...
      @apis.each.collect do |api|
        {
          ...
          short_description: MoonRaker.app.translate(api.short_description, lang),
    ...
    def to_json(lang = nil)
      {
        ...
        formats: formats,
        full_description: MoonRaker.app.translate(@full_description, lang),
    ...
    def api_data(dsl_data)
      ...
      if dsl_data[:api_from_routes]
        ...
        api_from_routes = MoonRaker.routes_for_action(resource.controller, method, desc: desc, options: options).map do |route_info|
          [route_info[:verb],
           route_info[:path],
           route_info[:desc],
    ...
    def load_recorded_examples
      (MoonRaker.recorded_examples[id] || [])
    ...
    def concern_subst(string)
      ...
      if from_concern?
        resource.controller._moon_raker_perform_concern_subst(string)
      else
        string
      end
    end
