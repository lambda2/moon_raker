module MoonRaker
  module Extractor
    class Collector
      ...
      def initialize
        @api_controllers_paths = MoonRaker.api_controllers_paths
      ...
      def controller_full_path(controller)
        MoonRaker::Extractor.controller_path controller.controller_path
      end

      def ignore_call?(record)
        return true unless record[:controller]
        return true if @ignored.include?(record[:controller].name)
        return true if @ignored.include?("#{MoonRaker.get_resource_name(record[:controller].name)}##{record[:action]}")
      ...
      def refine_params_description(params_desc, recorded_params)
        recorded_params.each do |key, value|
          ...
          if value.nil?
            ...
            if param_desc[:type].first == :number && (key.to_s !~ /id$/ || !MoonRaker::Validator::NumberValidator.validate(value))
              param_desc[:type].shift
            end
          end
      ...
      def add_routes_info(desc)
        ...
        desc[:api] = MoonRaker::Extractor.apis_from_routes[[desc[:controller].name, desc[:action]]]
        if desc[:api]
