  ...
  module Extractor
    class Writer
      ...
      def self.examples_file
        File.join(Rails.root, MoonRaker.configuration.doc_path, 'moon_raker_examples.json')
      end
      ...
      def deep_merge_examples(new_examples, old_examples)
        new_examples.map do |new_example|
          ...
          if old_example = old_examples.find { |old_example| old_example['verb'] == new_example_ordered['verb'] && old_example['versions'] == new_example_ordered['versions'] && old_example['query'] == new_example_ordered['query'] }
            ...
            new_example[:show_in_doc] = old_example['show_in_doc'] if MoonRaker.configuration.persist_show_in_doc && old_example['show_in_doc'].to_i > 0
      ...
      def load_new_examples
        @collector.records.reduce({}) do |h, (method, calls)|
          ...
          recorded_examples = calls.map do |call|
            method_descriptions = MoonRaker.get_method_descriptions(call[:controller], call[:action])
            call[:versions] = method_descriptions.map(&:version)

            if MoonRaker.configuration.show_all_examples
              show_in_doc = 1
    ...
    class ActionDescriptionUpdater
      ...
      def generated?
        old_header.include?(MoonRaker.configuration.generated_doc_disclaimer)
      end

      def update_apis(apis)
        new_header = ''
        new_header << MoonRaker.configuration.generated_doc_disclaimer << "\n" if generated?
        new_header << generate_apis_code(apis)
        new_header << ensure_line_breaks(old_header.lines).reject do |line|
          line.include?(MoonRaker.configuration.generated_doc_disclaimer) ||
            line =~ /^api/
        end.join
      ...
      def old_header
        ...
        @old_header = lines_above_method[/^\s*?#{Regexp.escape(MoonRaker.configuration.generated_doc_disclaimer)}.*/m]
      ...
      def controller_path
        @controller_path ||= MoonRaker::Extractor.controller_path(@controller.controller_path)
      end
      ...
      def generate_code(desc)
        code = "#{MoonRaker.configuration.generated_doc_disclaimer}\n"
    ...
    # Used to keep moon_raker_examples.yml params in order
